...
function clickOption(selector, parent = document) {
  var check = parent.querySelector('img[alt="Checkmark Image"], img[alt="Tick Mark Image"]'); // check if the currentQuestionContainer have a 'Checkmark Image'
  if (check != undefined) {
    window.close();
  }
...
function clickLoop() {
  if (prefs.autoClick) {
    click('#rqStartQuiz'); //click start playing (ext)
    clickOption('#currentQuestionContainer .b_cards[iscorrectoption=True]:not(.btsel)'); // click the correct answer in reward page having overlayPanel class
...
// && (window.sessionStorage['_mac'] == window.atob(prefs.defaultPhysicalAddress))